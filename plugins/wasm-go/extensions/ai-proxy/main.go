...
func onHttpRequestHeader(ctx wrapper.HttpContext, pluginConfig config.PluginConfig) types.Action {
	...
	} else if providerConfig.AllowsProtocolAutoDetect() {
		// Auto-detect protocol based on request path and handle conversion if needed.
		// Explicit "openai" keeps the existing Claude /v1/messages fallback behavior.
		// Explicit "anthropic" also reuses this Claude conversion chain when the provider
		// does not support Anthropic natively.
