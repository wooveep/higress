...
CHART_DIR="${CHART_DIR:-${SCRIPT_DIR}/higress}"
...
Options:
  --values <path>         Helm values file (default: helm/higress/values-local-minikube.yaml)
...
add(images, get(values, "higress-console", "image", "repository"), get(values, "higress-console", "image", "tag"))
...
add(images, get(values, "aigateway-portal", "backend", "image", "repository"), get(values, "aigateway-portal", "backend", "image", "tag"))
add(images,
...
if [[ "${SKIP_LOAD}" != "true" ]]; then
  ...
  for image in "${IMAGES[@]}"; do
    ...
    run minikube "${MINIKUBE_ARGS[@]}" image load "${image}"
  done
fi
